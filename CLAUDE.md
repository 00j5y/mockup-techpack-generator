...
10. **Dans le bloc header du techpack, les valeurs saisies sont en noir `#231F20` et en bold**, comme les libellés (`TP_COLORS.value` dans `headerLayout.ts`). Demande de Jay du 2026-07-28. Seul l'encadré rouge des tailles d'échantillon (`SIZE RANGE:`) reste rouge. **Partout ailleurs dans le techpack (pages 2 à 12 : cotes, lettres POI, pins, tableau page 3), la règle reste celle du rouge** : tout ce que l'utilisateur renseigne est en rouge `#FF0000`, toute la structure du template en `#231F20` / `#CCCCCC`, sauf le texte des cellules BOM, en noir. Ces pages ne sont pas encore construites (Phases 3 et 4) : si Jay veut du noir ailleurs qu'au header, ce sera une décision explicite à reprendre à ce moment-là, pas une extrapolation silencieuse de cette règle.
